...
def main(): #leadtime): #, model_n, thrshld, ts, neighbourhood):
    ...
    files = [f'{radar_dir}201909281805_nimrod_ng_radar_rainrate_composite_1km_UK'] 

    timesteps = range(0, 65, 5)
    ens = [[],[],[],[],[],[],[],[],[],[],
           [],[],[],[],[],[],[],[],[],[],
           [],[],[],[],[],[],[],[],[],[]]
    fss_p_lst = []
    ...
    for leadtime in leadtimes:
                ...
                for ens_n in range(30):
                    ...
                    if skip == False:
                        ...
                        e_nn_fraction = generate_fractions(nn_cube, n_size=neighbourhood,
                                                           threshold=thrshld)
                        fbs_e, fbs_e_worst = calculate_fbs(ob_fraction, e_nn_fraction)
                        fss_e0_nn = 1 - fbs_e / fbs_e_worst
                        ens[ens_n].append(fss_e0_nn)
                        print('ENS {} FSS: {}'.format(ens_n, fss_e0_nn))

                        ens_nn_fraction += e_nn_fraction
                    else:
                        print('ens file missing')
        ...
        fss_p_lst.append(fss_p)
        fss_e_nn_lst.append(fss_e_nn)

        print(leadtime, count, fss_p, fss_e_nn)
    plot_fss(timesteps, ens, fss_p_lst, fss_e_nn_lst, thrshld)

def plot_fss(timestep, ens, fss_p_lst, fss_e_nn_lst, thrshld):
    #pdb.set_trace()
    ## Plot timeseries
    plt.plot(timestep, fss_p_lst, linestyle='-.', label='Persistence')
    #plt.plot(timestep, fss_e_nn_lst, linestyle='-.', label='Full ensemble')
    for ens_mem, ens_n in enumerate(ens):
        plt.plot(timestep, ens_n, linestyle='-', label='Member {}'.format(ens_mem))

    plt.xlabel('Time (minutes)')
    plt.ylabel('FSS')
    plt.title('{} mm/hr threshold'.format(thrshld)) #, 9x9 neighbourhood')
...
def load_radar(dt, sample_points, leadtime, domain, ts):
    # Load radar data
    dt_str = (dt + timedelta(minutes = leadtime-ts)).strftime('%Y%m%d%H%M')
