...
def main(startdate): #, enddate, mod, n_past, n_future, ts=5):
    ...
    mod = 'model3256311' #model624800' #model141021' #model624800' #1755653' #model1734435'
    ts = 5
    ...
    while True:
        ...
        if skip == False:
            ...
            iris.save(pred_cube, "/data/cr1/cbarth/phd/SVG/model_output/synthetic_plots4_nn_{}.nc".format(mod, mod))
...
def prep_data(files, n_eval, batch_size):
    ...
    if len(data) < n_eval:
        ...
        for t in range(24):
             #data[t][t*5+50:t*5+70, 50:70] = 0.5
             #data[t][t*5+53:t*5+67, 53:67] = 1.
             #data[t][t*5+55:t*5+65, 55:65] = 2.
