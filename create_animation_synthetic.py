...
def main():
    #--------------------------------------------------------------
    # Options:
    dt_str = '201911020900' #201908140730' #201908140715' #201908121100' #201910011500' #202008271800' #201909281800' #07191300' #10272345' #291300' #201909191200' #201908141730' #201907260000' #201909282100' #201910011200' #201909281300' #201908141630'
    prior = 'lp'
    ...
    animate(r_cubelist, n_cubelist, nn_cubelist, dt_str, prior, model)
    #plotting(r_cubelist, n_cubelist, nn_cubelist, dt_str, prior, model)
    #for t in range(12):
    #    plot_synthetic(nn_cubelist3, r_cubelist, model, t)
    #animate_synthetic(r_cubelist, nn_cubelist, dt_str, prior, model)
...
def animate(r_cubelist, n_cubelist, nn_cubelist, dt_str, prior, model): #, nn_cubelist2):
    ...
    # Set video filename
    filename = "animations/{}_radar_animation2_{}_{}.mp4".format(prior, dt_str, model[:-4])
    ...
    with writer.saving(fig, filename, 300):
        for t in range(12):
            ...
            #cf = iplt.contourf(n_cubelist[0][t+1], levels, colors=colors, origin='lower', extend='max') #, cmap=cm) #, vmin=0, vmax=16)
            #cf.cmap.set_over('white')
            plt.gca().coastlines('50m', color='white')
            #for name, lat, lon in airport_data:
            #    plt.plot(lon, lat, marker="o", markerfacecolor="white", markeredgecolor="white", transform=ccrs.PlateCarree())
...
def animate_synthetic(r_cubelist, nn_cubelist, dt_str, prior, model):
    # define colours and levels for colorbar
    colors = ['black', 'cornflowerblue', 'royalblue', 'blue', 'lime', 'yellow', 'orange', 'red', 'fuchsia'] #, 'white']
    levels = [0, 0.1, 0.25, 0.5, 1., 2., 4., 8. ,16., 32.]
    ...
    # Set video filename
    filename = "animations/{}_radar_animation_we3_{}.mp4".format(prior, model[:-4])
    ...
    with writer.saving(fig, filename, 300):
        for t in range(12):#21): #8):
            ...
            fig.suptitle('T+{:02d} min'.format(t*5), fontsize=30)
            # Save frame
            writer.grab_frame()
    plt.close()
    print('finished animating')

def plot_synthetic(nn_cubelist, r_cubelist, model, t): #, nn_cubelist2):
    # define colours and levels for colorbar
    colors = ['black', 'cornflowerblue', 'royalblue', 'blue', 'lime', 'yellow', 'orange', 'red', 'fuchsia'] #, 'white']
    levels = [0, 0.1, 0.25, 0.5, 1., 2., 4., 8. ,16., 32.]
    fig = plt.figure(figsize=(12, 6))
    # Add axes to the figure, to place the colour bar [left, bottom, width, height] (of cbar)
    colorbar_axes = fig.add_axes([0.15, 0.1, 0.73, 0.03])
    fname = "animations/synthetic_radar_animation_we_t{}_{}.png".format(t, model[:-4])
    print('time = ', t*5) #(t+1)*5) #15)
    ax = fig.add_subplot(1,2,1)
    #pdb.set_trace()  # adding [0:64, 64:128] to reduce domain to focus on SE England
    cf = iplt.contourf(r_cubelist[t], levels, colors=colors, origin='lower', extend='max') #, cmap=cm) #, vmin=0, vmax=16)
    #cf.cmap.set_over('white')
    ...
    plt.title('Radar', fontsize=18)
    ax = fig.add_subplot(1,2,2)
    print(nn_cubelist[0][t])
    nn_cube = nn_cubelist[0][t] #* 1.5  #fix scaling issue by adding multiplicative factor
    cf = iplt.contourf(nn_cube, levels, colors=colors, origin='lower', extend='max') #, cmap=cm) #, vmin=0, vmax=16)
    #cf.cmap.set_over('white')
    #plt.gca().coastlines('50m', color='white')
    plt.title('Synthetic data prediction', fontsize=18)
...
def plotting(r_cubelist, n_cubelist, nn_cubelist, dt_str, prior, model): #, nn_cubelist2):
    # define colours and levels for colorbar
    colors = ['black', 'cornflowerblue', 'royalblue', 'blue', 'lime', 'yellow', 'orange', 'red', 'fuchsia'] #, 'white']
    levels = [0, 0.1, 0.25, 0.5, 1., 2., 4., 8. ,16., 32.]
    fig = plt.figure(figsize=(12, 5))
    ...
    colorbar_axes = fig.add_axes([0.15, 0.1, 0.73, 0.03])
    t = 5
    fname = "animations/{}_radar_animation_{}_t{}_{}.png".format(prior, dt_str, (t+1)*5, model[:-4])
    print('time = ', (t+1)*5) #15)
    ...
    #cf.cmap.set_over('white')
    plt.gca().coastlines('50m', color='white')
    plt.title('All training data', fontsize=18)
    ...
    #cf.cmap.set_over('white')
    plt.gca().coastlines('50m', color='white')
    plt.title('50% training data', fontsize=15)
...
def load_nn_pred(dt_str, model, domain, nn_datadi):
    ...
    #print(nn_cube)
    for t in range(25): #[3, 6, 9, 12, 15, 18, 21, 24]:
...
def load_radar(dt, dt_str, sample_points, domain, datadi):
    # Load radar data
    r_cubelist = []
    for t, mi in enumerate(range(0, 120, 5)):
        ti = (dt + timedelta(minutes = mi)).strftime('%Y%m%d%H%M')
        ...
        # #synthetic data
        # radar_cube.data[np.where(radar_cube.data >= 0)] = 0.
        #
        # # s -> n direction
        # #radar_cube.data[t*5+40:t*5+100, 30:90] = 0.5
        # #radar_cube.data[t*5+50:t*5+90, 40:80] = 1.
        # #radar_cube.data[t*5+60:t*5+80, 50:70] = 2.
        # #radar_cube.data[t*5+65:t*5+75, 55:65] = 4.
        # #radar_cube.data[t*5+69:t*5+71, 59:61] = 8.
        #
        # # w -> e direction
        # radar_cube.data[30:90, t*5+40:t*5+100] = 0.5
        # radar_cube.data[40:80, t*5+50:t*5+90] = 1.
        # radar_cube.data[50:70, t*5+60:t*5+80] = 2.
        # radar_cube.data[55:65, t*5+65:t*5+75] = 4.
        # radar_cube.data[59:61, t*5+69:t*5+71] = 8.
        #
        # #smaller square: south -> north
        # # radar_cube.data[t*5+55:t*5+85, 45:75] = 0.5
        # # radar_cube.data[t*5+60:t*5+80, 50:70] = 1.
        # # radar_cube.data[t*5+65:t*5+76, 55:66] = 2.
        # # radar_cube.data[t*5+67:t*5+73, 57:63] = 4.
        # # radar_cube.data[t*5+69:t*5+71, 59:61] = 8.
