...
def prob_animate(r_cubelist, ens_cubes, dt_str, prior):
    ...
    with writer.saving(fig, filename, 300):
        for t in range(14): #8):
            ...
            # Plot radar data
            cf = iplt.contourf(r_cubelist[t][0:74, 54:128] , levels, colors=colors, origin='lower', extend='max')
            ...
            cf = iplt.contourf(prob_cube[0:74, 54:128], levels=levels2) #, colors=colors, origin='lower', extend='max') #, cmap=cm) #, vmin=0, vmax=16)
            cf.cmap.set_over('white')
            plt.gca().coastlines('50m', color='white')
            plt.title('Prob > {} mm/hr'.format(threshold), fontsize=18)
            cbar = plt.colorbar(cf, orientation='horizontal')
            cbar.ax.set_xlabel('%')
