...
def main(startdate): #, enddate, mod, n_past, n_future, ts=5):
    ...
    while True:
        ...
        if skip == False:
            ...
            iris.save(pred_cube, "/data/cr1/cbarth/phd/SVG/model_output/{}/plotsC_nn_T{}_{}.nc".format(mod, dt_str, mod))
...
def prep_data(files, n_eval, batch_size):
    ...
    if len(data) < n_eval:
        ...
        #-----------------------------------------------
        #Test C - create synthetic empty data
        data[np.where(data > 0)] = 0.
        #------------------------------------------
        # Normalise data
        data = data / 64.
        ...
        loader = DataLoader(tensor, batch_size=batch_size, shuffle=False, pin_memory=True) #batch_size=1)
...
def make_gifs(x, name, frame_predictor, posterior, n_eval, encoder, decoder,
    ...
    #----------------------------------------------------------------------
    ##Test B to input the same 3 frames
    #x[1] = x[0]
    #x[2] = x[0]
    #----------------------------------------------------------
    ##Test C to input synthetic box
    #pdb.set_trace()
    x[0][0][0][30:40, 50:60] = 1
    x[1][0][0][30:40, 55:65] = 1
    x[2][0][0][30:40, 60:70] = 1
...
if __name__ == "__main__":
    ...
    startdates = []
    startdate = datetime.strptime('201904012100', '%Y%m%d%H%M')   #05  #09
    #startdate = datetime.strptime('202008271205', '%Y%m%d%H%M')
    for d in range (0, 1): #360): #1): #335): #30): #, 3):
        for m in range(96):
            startdates.append(startdate + timedelta(days = d) + timedelta(minutes = m*15))

    startdates = [datetime.strptime('201912020900', '%Y%m%d%H%M')]
