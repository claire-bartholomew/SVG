...
def get_training_batch():
    while True:
        for sequence in train_loader:
            ...
            print('batch size = ', np.shape(batch))
            pdb.set_trace()
            yield batch
...
def plot(x, epoch):
    ...
    for i in range(nrow):
        ...
        for s in range(nsample):
            mse = 0
            for t in range(opt.n_eval):
                mse +=  torch.sum( (gt_seq[t][i].data - gen_seq[s][t][i].data)**2 )
            if mse < min_mse:
                min_mse = mse
                min_idx = s
...
def train(x):
    ...
    return mse.data.numpy()/(opt.n_past+opt.n_future), kld.data.numpy()/(opt.n_future+opt.n_past)
